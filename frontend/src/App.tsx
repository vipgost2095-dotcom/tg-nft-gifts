...
import { beginCell, Address } from '@ton/core'; // <-- Импортируем инструменты для создания payload
import './i18n';
...
export default function App() {
  ...
  const handleMint = async () => {
    ...
    try {
      // 1. Формируем ПРАВИЛЬНЫЙ payload для cNFT
      // OpCode 0x18 - это стандартная команда "mint" для большинства контрактов TON
      const MINT_PRICE = '10000000'; // 0.01 TON в нанотонах
      const GAS_FEE = '30000000';    // 0.03 TON на газ (cNFT может требовать чуть больше)
      
      // Конвертируем адрес из URL-safe формата (с дефисами) в стандартный base64
      // useTonAddress() возвращает формат UQ...-..., а @ton/core требует EQ...+...
      const safeAddress = userAddress.replace(/-/g, '+').replace(/_/g, '/');
      const ownerAddr = Address.parse(safeAddress);
      
      // Создаем тело сообщения (payload)
      const mintBody = beginCell()
        .storeUint(0x18, 32)       // op::mint (стандартный opcode)
        .storeUint(0, 64)          // query_id
        .storeAddress(ownerAddr)   // адрес владельца NFT
        .endCell();
        ...
        messages: [
          {
            ...
            payload: mintBody.toBoc().toString('base64') // <-- Правильный payload
          }
        ]
      });
